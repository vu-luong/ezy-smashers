package org.youngmonkeys.ezysmashers.app.constant;

public final class GameConstants {

    public static final float CLIENT_FIXED_DELTA_TIME = 0.025f;
    public static final int MAX_HISTORY_SIZE = 10;
    public static final float ATTACK_RANGE_UPPER_BOUND = 2.0f;
    public static final float HAMMER_DISTANCE_UPPER_BOUND = 2.0f;
    public static final float MAP_MAX_X = 20;
    public static final float MAP_MAX_Y = 1;
    public static final float MAP_MAX_Z = 20;
    public static final float MAP_CELL_SIZE = 1;
    public static final float MAP_BORDER_OFFSET = 0.4f;

    private GameConstants() {}
}
